...
@section('content_dashboard')
    ...
    <div class="container">
        <div class="row">
            ...
            <h3 class="dash-main-title text-center col-12 wow fadeIn">{{trans('local.hello_in').trans('local.subcategories_for')." "}}<span class="dash-title-span">{{$category->name}}</span></h3>
            ...
            <div class="category-table wow fadeIn col-12">
                ...
                <table id="example" class="table datatable-save-state table-striped table-bordered table-responsive-sm">
                    ...
                    @foreach($sub_categories as $sub_category)
                        <tr>
                            <td>{{$i++}}</td>
                            <td><a href="{{route('category',$category->name)}}" style="color: blue;">{{$sub_category->name}}</a></td>
